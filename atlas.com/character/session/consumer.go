...
func StatusEventConsumer(l logrus.FieldLogger) func(groupId string) consumer.Config {
	return func(groupId string) consumer.Config {
		return kafka.NewConfig(l)(consumerStatusEvent)(EnvEventTopicSessionStatus)(groupId)
	}
}

func StatusEventRegister(l logrus.FieldLogger, db *gorm.DB) (string, handler.Handler) {
	return kafka.LookupTopic(l)(EnvEventTopicSessionStatus), message.AdaptHandler(message.PersistentConfig(handleStatusEvent(db)))
}
